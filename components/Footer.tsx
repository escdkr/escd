...
// Custom 'The Neon Shard' Icon - Luxury Crystal Edition (Footer)
const DaggerIcon = ({ className, ...props }: React.ComponentProps<'svg'>) => (
  ...
  >
    ...
    <g>
      {/* Full Body Silhouette - Tactical Proportions */}
      <path 
      ...
      {/* Center Refraction Line (Crystal reflection) */}
      <path
        d="M12 10 L12 21.5"
        stroke="white"
        strokeWidth="0.2"
...
interface FooterProps {
  ...
  onReviewsClick?: () => void; // Added Prop
}

export const Footer: React.FC<FooterProps> = ({ onFAQClick, onLegalClick, onCurriculumClick, onReviewsClick }) => {
  return (
    <footer className="border-t border-white/5 bg-brand-charcoal pt-20 pb-10 mt-auto relative overflow-hidden">
      {/* Footer Ambient Glow - Midnight/Indigo */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="col-span-1 md:col-span-2 space-y-6">
            <div className="flex items-center gap-5">
               {/* The Neon Shard Icon - Rotate -45deg */}
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-sm text-brand-muted">
              <li><button onClick={onCurriculumClick} className="hover:text-[#FBBF24] transition-colors">커리큘럼</button></li>
              <li><a href="#" className="hover:text-[#FBBF24] transition-colors">가격 정책</a></li>
              <li><button onClick={onReviewsClick} className="hover:text-[#FBBF24] transition-colors">독자 후기</button></li>
              <li><button onClick={onFAQClick} className="hover:text-[#FBBF24] transition-colors">자주 묻는 질문</button></li>
            </ul>
          </div>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2 text-sm text-brand-muted">
              <li><button onClick={onLegalClick} className="hover:text-[#FBBF24] transition-colors">개인정보 처리방침</button></li>
              <li><button onClick={onLegalClick} className="hover:text-[#FBBF24] transition-colors">이용 약관</button></li>
              <li><button onClick={onLegalClick} className="hover:text-[#FBBF24] transition-colors">환불 정책</button></li>
            </ul>
          </div>
        </div>
